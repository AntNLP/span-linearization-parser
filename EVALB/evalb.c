...
void print_head() {
  printf(
      "  Sent.                        Matched  Bracket   Cross        Correct "
      "Tag\n");
  printf(
      " ID  Len.  Stat. Recal  Prec.  Bracket gold test Bracket Words  Tags "
      "Accracy\n");
  printf(
      "========================================================================"
      "====\n");
...
/* parameter file */
/*----------------*/
void read_parameter_file(filename) char *filename;
{
...
#define STRNCMP(s)                      \
  (strncmp(param, s, strlen(s)) == 0 && \
   (param[strlen(s)] == '\0' || isspace(param[strlen(s)])))

void set_param(param, value) char *param, *value;
{
  ...
  if (STRNCMP("DEBUG")) {
    ...
    Delete_label_for_length[Delete_label_for_length_n] =
...
/*-----------------------------*/
int read_line(buff, terminal, quotterm, wn, bracket, bn)
char *buff;
s_terminal terminal[];
s_term_ind quotterm[];
int *wn;
  ...
  for (p = buff, qt = 0, wid = 0, bid = 0; *p != '\0';) {
    if (isspace(*p)) {
      ...
      if (isspace(*q)) {
        ...
        if (DEBUG > 1) printf("label=%s, word=%s, wid=%d\n", label, word, wid);
        /* quote terminal */
...
void calc_result(unsigned char *buf1, unsigned char *buf) {
  ...
  for (i = 0; i < wn1; i++) {
    if (word_comp(terminal1[i].word, terminal2[i].word) == 0) {
      Error("Words unmatch (%s|%s)\n", terminal1[i].word, terminal2[i].word);
      individual_result(0, 0, 0, 0, 0, 0);
      return;
    }
  }
  ...
  for (i = 0; i < bn1; i++) {
    for (j = 0; j < bn2; j++) {
      if (DEBUG > 1)
        printf(
            "1.res=%d, 2.res=%d, 1.start=%d, 2.start=%d, 1.end=%d, 2.end=%d\n",
            bracket1[i].result, bracket2[j].result, bracket1[i].start,
            bracket2[j].start, bracket1[i].end, bracket2[j].end);

      // does bracket match?
      if (bracket1[i].result != 5 && bracket2[j].result == 0 &&
  ...
  for (j = 0; j < bn2; j++) {
    for (i = 0; i < bn1; i++) {
      if (bracket1[i].result != 5 && bracket2[j].result != 5 &&
          ((bracket1[i].start < bracket2[j].start &&
        ...
        if (i != last_i) {
          if (DEBUG > 1) {
            printf("  CROSSING[%d-%d]: ", bracket1[i].start,
                   bracket1[i].end - 1);
            l = bracket1[i].buf_end - bracket1[i].buf_start;
            strncpy(my_buf, buf1 + bracket1[i].buf_start, l);
            my_buf[l] = '\0';
            ...
            printf("  GOLD (tokens %d-%d):
            ",bracket1[i].start,bracket1[i].end-1); l =
            bracket1[i].buf_end-bracket1[i].buf_start;
            strncpy(my_buf,buf1+bracket1[i].buf_start,l);
            my_buf[l] = '\0';
            printf("%s\n",my_buf);
            */
  ...
  if (DEBUG > 1) {
    for (i = 0; i < MAX_QUOTE_TERM; i++) {
      if (quotterm1[i].index != -1)
        printf("%d: %s - %s\n", quotterm1[i].index, quotterm1[i].term.label,
               quotterm1[i].term.word);
      if (quotterm2[i].index != -1)
        printf("%d: %s - %s\n", quotterm2[i].index, quotterm2[i].term.label,
    ...
    if (ind != -1) {
      for (j = 0; j < MAX_QUOTE_TERM; j++) {
        if (quotterm1[j].index == ind &&
            strcmp(quotterm1[j].term.label, quotterm2[i].term.label) != 0) {
          if (is_deletelabel(quotterm1[j].term.label) &&
              !is_deletelabel(quotterm2[i].term.label)) {
            reinsert_term(&quotterm1[j], terminal1, bracket1, &wn1);
            for (k = j; k < MAX_QUOTE_TERM; k++)
              if (quotterm1[k].index != -1) quotterm1[k].index++;
          } else if (is_deletelabel(quotterm2[i].term.label) &&
                     !is_deletelabel(quotterm1[j].term.label)) {
            reinsert_term(&quotterm2[i], terminal2, bracket2, &wn2);
            for (k = i; k < MAX_QUOTE_TERM; k++)
              if (quotterm2[k].index != -1) quotterm2[k].index++;
          }
        }
      }
    } else
...
void reinsert_term(quot, terminal, bracket, wn) s_term_ind *quot;
s_terminal terminal[];
s_bracket bracket[];
int *wn;
{
...
/*------------------------*/
void modify_label(label) char *label;
{
  char *p;
...
/*-----------------------------------------------*/
void individual_result(wn1, bn1, bn2, match, crossing, correct_tag) int wn1,
    bn1, bn2, match, crossing, correct_tag;
{
  /* Statistics for ALL */
  /*--------------------*/
  TOTAL_sent++;
  ...
  printf("%6.2f %6.2f   %3d    %3d  %3d    %3d",
         ...
         (r_bn2 == 0 ? 0.0 : 100.0 * match / r_bn2), match, r_bn1, r_bn2,
         crossing);
...
void print_total() {
  ...
  if (TOTAL_bn1 > 0 && TOTAL_bn2 > 0) {
    printf("                %6.2f %6.2f %6d %5d %5d  %5d",
           ...
           (TOTAL_bn2 > 0 ? 100.0 * TOTAL_match / TOTAL_bn2 : 0.0), TOTAL_match,
           TOTAL_bn1, TOTAL_bn2, TOTAL_crossing);
  }

  printf("  %5d %5d   %6.2f", TOTAL_word, TOTAL_correct_tag,
  ...
  for (i = 0; i < n; i++) {
    if (bracket1[i].start != -1) {
      printf("%3d : %d : %3d  %3d  %-6s      ", i, bracket1[i].result,
             bracket1[i].start, bracket1[i].end, bracket1[i].label);
    } else {
      printf("                                ");
    }
    ...
    if (bracket2[i].start != -1) {
      printf("%3d : %d : %3d  %3d  %-6s\n", i, bracket2[i].result,
             bracket2[i].start, bracket2[i].end, bracket2[i].label);
    } else {
      printf("\n");
    }
  }
...
int is_deletelabel_for_length(s)
char *s;
{
  int i;
...
char *s1, *s2;
{
  ...
  for (i = 0; i < EQ_word_n; i++) {
    if ((strcmp(s1, EQ_word[i].s1) == 0 && strcmp(s2, EQ_word[i].s2) == 0) ||
        (strcmp(s1, EQ_word[i].s2) == 0 && strcmp(s2, EQ_word[i].s1) == 0)) {
      return (1);
    }
  }
...
char *s1, *s2;
{
  ...
  for (i = 0; i < EQ_label_n; i++) {
    if ((strcmp(s1, EQ_label[i].s1) == 0 && strcmp(s2, EQ_label[i].s2) == 0) ||
        (strcmp(s1, EQ_label[i].s2) == 0 && strcmp(s2, EQ_label[i].s1) == 0)) {
      return (1);
    }
  }
...
/* errors */
/*--------*/
void Error(s, arg1, arg2, arg3) char *s, *arg1, *arg2, *arg3;
{
  Status = 1;
...
/* fatal error to exit */
/*---------------------*/
void Fatal(s, arg1, arg2, arg3) char *s, *arg1, *arg2, *arg3;
{
  fprintf(stderr, s, arg1, arg2, arg3);
  exit(1);
}
...
void Usage() {
  ...
  fprintf(stderr,
          "    -d             debug mode                            \n");
  fprintf(stderr,
          "    -D             debug mode plus bracketing info       \n");
  fprintf(stderr,
          "    -c n           cut-off length forstatistics (def.=40)\n");
  fprintf(stderr,
          "    -e n           number of error to kill (default=10)  \n");
  fprintf(stderr,
          "    -p param_file  parameter file                        \n");
