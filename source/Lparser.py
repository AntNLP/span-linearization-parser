...
class MultiHeadAttention(nn.Module):
    ...
    def pad_and_rearrange(self, q_s, k_s, v_s, batch_idxs):
        ...
        q_padded = q_s.data.new(n_head, mb_size, len_padded, d_k).fill_(0.0)
        k_padded = k_s.data.new(n_head, mb_size, len_padded, d_k).fill_(0.0)
        v_padded = v_s.data.new(n_head, mb_size, len_padded, d_v).fill_(0.0)
        ...
        for i, (num_embeddings, emb_dropout) in enumerate(
            ...
            if hparams.use_cat:
                if i == len(num_embeddings_list) - 1:
                    ...
                    emb = nn.Embedding(
                        num_embeddings,
                        self.d_content // cun - self.pretrain_dim,
                        **kwargs
                    )
                else:
    ...
    def forward(self, xs, pre_words_idxs, batch_idxs, extra_content_annotations=None):
        ...
        if self.pretrain_dim != 0:
            content_annotations = torch.cat(
                [
                    content_annotations,
                    self.pretrain_emb_dropout(
                        self.pretrain_emb(pre_words_idxs), batch_idxs
                    ),
                ],
                dim=1,
            )
...
def get_bert(bert_model_path, bert_do_lower_case):
    ...
    tokenizer = BertTokenizer.from_pretrained(
        bert_model_path, do_lower_case=bert_do_lower_case
    )
...
class BiAAttention(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.weight = nn.Parameter(
            torch_t.FloatTensor(hparams.d_biaffine + 1, hparams.d_biaffine + 1)
        )
    ...
    def forward(self, input_d, input_e):
        ...
        score = torch.matmul(
            torch.cat(
                [
                    input_d,
                    torch_t.FloatTensor(input_d.size(0), 1)
                    .fill_(1)
        ...
        score1 = torch.matmul(
            ...
            torch.transpose(
                torch.cat(
                    [
                        input_e,
                        torch_t.FloatTensor(input_e.size(0), 1)
                        .fill_(1)
                        .requires_grad_(False),
                    ],
                    dim=1,
...
class Encoder(nn.Module):
    ...
    def forward(self, xs, pre_words_idxs, batch_idxs, extra_content_annotations=None):
        ...
        if self.hparams.partitioned:
            ...
            res_pack_seq = torch.nn.utils.rnn.pack_sequence(
                res_list, enforce_sorted=False
            )
            lstm_out, _ = self.lstm(res_pack_seq)
            res_pad, seq_len = torch.nn.utils.rnn.pad_packed_sequence(
                lstm_out, batch_first=True
            )

            res = torch.cat(
                [res_pad[i, : seq_len[i].data, :] for i in range(seq_len.size(0))],
                dim=0,
            )
...
class ChartParser(nn.Module):
    def __init__(
        ...
        if hparams.use_bert or hparams.use_bert_only:
            self.bert_tokenizer, self.bert = get_bert(
                hparams.bert_model_path, hparams.bert_do_lower_case
            )
            if hparams.bert_transliterate:
                from transliterate import TRANSLITERATIONS
    ...
    def parse_batch(self, sentences, golds=None):
        ...
        if self.char_encoder is not None:
            ...
            extra_content_annotations_list.append(
                self.char_encoder(char_idxs_encoder, word_lens_encoder, batch_idxs)
            )
        ...
        if self.xlnet is not None:
            ...
            for snum, sentence in enumerate(sentences):
                ...
                if self.hparams.pad_left:
                    all_input_ids[
                        snum, self.xlnet_max_len - len(input_ids) :
                    ] = input_ids
                    all_input_mask[
                        snum, self.xlnet_max_len - len(input_mask) :
                    ] = input_mask
                    all_word_start_mask[
                        snum, self.xlnet_max_len - len(word_start_mask) :
                    ] = word_start_mask
                    all_word_end_mask[
                        snum, self.xlnet_max_len - len(word_end_mask) :
                    ] = word_end_mask
                else:
                    all_input_ids[snum, : len(input_ids)] = input_ids
                    all_input_mask[snum, : len(input_mask)] = input_mask
        ...
        if self.encoder is not None:
            ...
            fencepost_annotations_start = features.masked_select(
                all_word_start_mask.to(torch.bool).unsqueeze(-1)
            ).reshape(-1, features.shape[-1])
        ...
        h_span_left, h_span_right, h_label_left, h_label_right = self.spanrep(
        ...
        for i, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
            ...
            if is_train:
                i_pred, j_pred = self.get_spans_by_lbound(
                    lbound_pred_list, 0, len(lbound_pred_list) - 1
                )
                label_gold = [
                    self.label_vocab.index(golds[i].oracle_label(l, r))
                    for l, r in zip(i_pred, j_pred)
                ]

                h_label = h_label_r[j_pred] - h_label_l[i_pred]
                label_scores = self.f_label(h_label)
                label_loss = self.loss_fun(
                    label_scores, from_numpy(np.array(label_gold)).requires_grad_(False)
                )

                lbound_gold = [leaf.lbound for leaf in golds[i].leaves()]
                lbound_loss = self.loss_fun(
                    lbound_att, from_numpy(np.array(lbound_gold)).requires_grad_(False)
                )

                loss = loss + lbound_loss + label_loss
            else:
                tree = self.decode_tree_by_lbound(
                    lbound_pred_list, h_label_l, h_label_r, sentences[i]
                )
                trees.append(tree)
    ...
    def decode_tree_by_lbound(self, lbound, h_leftspan, h_rightspan, sentence):
        ...
        root_label_index = (
            (torch.argmax(label_scores[:1, 1:], dim=1) + 1).cpu().data.numpy().tolist()
        )
